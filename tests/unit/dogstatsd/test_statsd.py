...
def nbytes(payload):
    """Wire size of a payload: the telemetry byte counters count bytes, not characters."""
...
class TestDogStatsd(unittest.TestCase):
    ...
    def assert_equal_telemetry(self, expected_payload, actual_payload, telemetry=None, **kwargs):
        if telemetry is None:
            telemetry = telemetry_metrics(bytes_sent=nbytes(expected_payload), **kwargs)
    ...
    def send_and_assert(
        ...
        for message in expected_messages:
            ...
            expected_metrics=telemetry_metrics(
                ...
                bytes_sent=nbytes(message) + last_telemetry_size
            )
            ...
            last_telemetry_size = nbytes(expected_metrics)
    ...
    def test_gauge_constant_tags(self):
        ...
        self.assert_equal_telemetry(metric, self.recv(2), telemetry=telemetry_metrics(tags="bar:baz,foo", bytes_sent=nbytes(metric)))
...
async def print_foo():
    ...
    def test_batching(self):
        ...
        self.assert_equal_telemetry(
                expected,
                self.recv(2),
                telemetry=telemetry_metrics(metrics=2, bytes_sent=nbytes(expected))
        )
    ...
    def test_batching_sequential(self):
        ...
        expected_metrics1=telemetry_metrics(metrics=1, bytes_sent=nbytes(expected1))
        self.assert_equal_telemetry(
            expected1,
            self.recv(2),
        ...
        self.assert_equal_telemetry(
            ...
            telemetry=telemetry_metrics(
                metrics=2,
                packets_sent=2,
                bytes_sent=nbytes(expected2 + expected_metrics1)
            )
        )
    ...
    def test_telemetry(self):
        ...
        telemetry = telemetry_metrics(metrics=2, events=2, service_checks=3, bytes_sent=4 + nbytes(payload),
    ...
    def test_telemetry_bytes_sent_counts_bytes_not_characters(self):
        self.statsd._container_id = None

        self.statsd.gauge('page.views', 1, tags=[u'city:montréal'])

        payload = u'page.views:1|g|#city:montréal\n'
        # 30 characters, 31 bytes on the wire.
        self.assertEqual(30, len(payload))
        self.assert_equal_telemetry(
            payload,
            self.recv(2),
            telemetry=telemetry_metrics(bytes_sent=31),
        )

    def test_telemetry_bytes_dropped_writer_counts_bytes_not_characters(self):
        statsd = DogStatsd(telemetry_min_flush_interval=3600)
        ...
        self.assertEqual(31, statsd.bytes_dropped_writer)
        self.assertEqual(1, statsd.packets_dropped_writer)
    ...
    def test_telemetry_flush_interval_batch(self):
        ...
        metric = 'gauge1:1|g\ngauge2:2|g\n'
        self.assert_equal_telemetry(metric, fake_socket.recv(2), telemetry=telemetry_metrics(metrics=2, bytes_sent=nbytes(metric)))
        # assert that _last_flush_time has been updated
        self.assertTrue(time1 < dogstatsd._last_flush_time)
    ...
    def test_context_manager_restores_enabled_buffering_state(self):
        ...
        metrics_packet1 = telemetry_metrics(metrics=2, bytes_sent=nbytes(metrics1), packets_sent=1)
        self.assertEqual(metrics_packet1, fake_socket.recv(no_wait=True))

        metrics2 = '\n'.join([metric3, metric4]) + "\n"
        metrics_packet2 = telemetry_metrics(metrics=2, bytes_sent=nbytes(metrics_packet1 + metrics2), packets_sent=2)
    ...
    def test_context_manager_restores_disabled_buffering_state(self):
        ...
        metrics2 = '\n'.join([metric3]) + "\n"
        metrics_packet2 = telemetry_metrics(metrics=1, bytes_sent=nbytes(metrics_packet1 + metrics2), packets_sent=2)
        self.assertEqual(metrics2, fake_socket.recv())
        self.assertEqual(metrics_packet2, fake_socket.recv(no_wait=True))

        metrics3 = '\n'.join([metric4]) + "\n"
        metrics_packet3 = telemetry_metrics(metrics=1, bytes_sent=nbytes(metrics_packet2 + metrics3), packets_sent=2)
    ...
    def test_batched_buffer_autoflush(self):
        ...
        telemetry = telemetry_metrics(metrics=0, packets_sent=2, bytes_sent=nbytes(single_metric) + len(telemetry))
        self.assertEqual(telemetry, fake_socket.recv())
    ...
    def test_tags_from_environment(self):
        ...
        self.assertEqual(telemetry_metrics(tags="country:china,age:45,blue", bytes_sent=nbytes(metric)), dogstatsd.socket.recv())
    ...
    def test_tags_from_environment_and_constant(self):
        ...
        self.assertEqual(metric, dogstatsd.socket.recv())
        self.assertEqual(telemetry_metrics(tags=tags, bytes_sent=nbytes(metric)), dogstatsd.socket.recv())
    ...
    def test_entity_tag_from_environment(self):
        ...
        self.assertEqual(metric, dogstatsd.socket.recv())
        self.assertEqual(
            telemetry_metrics(tags="dd.internal.entity_id:04652bb7-19b7-11e9-9cc6-42010a9c016d", bytes_sent=nbytes(metric)),
            dogstatsd.socket.recv())
