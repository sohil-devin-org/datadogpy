...
def utf8_len(payload):
    """Size of a payload on the wire, which is what the bytes_* telemetry counts."""
...
class TestDogStatsd(unittest.TestCase):
    ...
    def send_and_assert(
        ...
        for message in expected_messages:
            ...
            last_telemetry_size = utf8_len(expected_metrics)
    ...
    def _statsd_without_telemetry_flush(self, **kwargs):
        statsd = DogStatsd(telemetry_min_flush_interval=3600, **kwargs)
        statsd.socket = FakeSocket()
        statsd._reset_telemetry()
        return statsd

    def test_telemetry_bytes_sent_counts_utf8_bytes(self):
        statsd = self._statsd_without_telemetry_flush()
        statsd.gauge('page.views', 1, tags=[u'city:montréal'])
        statsd.flush()

        wire = statsd.socket.payloads[0]
        self.assertEqual(b'page.views:1|g|#city:montr\xc3\xa9al\n', wire)
        self.assertEqual(31, len(wire))
        self.assertEqual(31, statsd.bytes_sent)
        self.assertEqual(1, statsd.packets_sent)

        statsd.gauge('page.views', 1, tags=[u'region:東京'])
        statsd.flush()
        self.assertEqual(b'page.views:1|g|#region:\xe6\x9d\xb1\xe4\xba\xac\n', statsd.socket.payloads[1])
        self.assertEqual(31 + 30, statsd.bytes_sent)
        self.assertEqual(2, statsd.packets_sent)

    def test_telemetry_bytes_dropped_writer_counts_utf8_bytes(self):
        statsd = self._statsd_without_telemetry_flush()
        statsd.socket = BrokenSocket(errno.ECONNREFUSED)
        statsd.gauge('page.views', 1, tags=[u'city:montréal'])
        statsd.flush()

        self.assertEqual(0, statsd.bytes_sent)
        self.assertEqual(31, statsd.bytes_dropped_writer)
        self.assertEqual(1, statsd.packets_dropped_writer)

    def test_telemetry_packet_size_counts_utf8_bytes(self):
        statsd = self._statsd_without_telemetry_flush(constant_tags=[u'région:québec'])
        statsd.gauge('page.views', 1)
        statsd.flush()
        self.assertEqual(b'page.views:1|g|#r\xc3\xa9gion:qu\xc3\xa9bec\n', statsd.socket.payloads[0])

        # Force a telemetry flush on the next packet: the telemetry payload itself
        # carries the multibyte constant tags, and is what bytes_sent holds after
        # _reset_telemetry().
        statsd._telemetry_flush_interval = 0
        statsd.gauge('page.views', 2)
        statsd.flush()

        telemetry = statsd.socket.payloads[2]
        self.assertTrue(telemetry.startswith(b'datadog.dogstatsd.client.metrics:'))
        self.assertEqual(len(telemetry), statsd.bytes_sent)
        self.assertEqual(1, statsd.packets_sent)
        self.assertNotEqual(len(telemetry.decode('utf-8')), len(telemetry))
...
async def print_foo():
    ...
    def test_telemetry(self):
        ...
        telemetry = telemetry_metrics(metrics=2, events=2, service_checks=3, bytes_sent=4 + utf8_len(payload),
